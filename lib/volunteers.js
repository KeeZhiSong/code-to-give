...
function toVolunteer(row) {
  return {
    ...
    pillars: row.pillars || [],
    roles: row.roles || [],
    // Loyalty ledger. Nothing writes it yet — it's carried through so it's
    // there when the programme is built.
...
/**
 * Credit a volunteer with having attended an event — the loyalty ledger
 * migration 003 added, so "re-invite past volunteers" (see lib/readiness.js)
 * has real people to suggest. Idempotent: marking the same event twice is a
 * no-op, not a duplicate entry.
 *
 * Deliberately a decision an organiser makes on the roster after the event,
 * not something derived from a "yes" RSVP — an RSVP that turned into a
 * no-show shouldn't earn a credit.
 */
export async function markEventAttended(phone, eventName) {
  ...
  if (!existing) throw new Error("No volunteer found for this phone number.");

  const current = existing.events_attended || [];
  if (current.includes(eventName)) {
    return { alreadyMarked: true, eventsAttended: current };
  }

  const eventsAttended = [...current, eventName];
  unwrap(
    await db()
      .from("volunteers")
  ...
  return { alreadyMarked: false, eventsAttended };
}
