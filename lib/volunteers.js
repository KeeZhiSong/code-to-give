// Volunteers — read from Supabase, topped up from the Google Form sheet.
//
// Division of labour:
//   Google Form → Sheet   the INTAKE. Zero build, works on any phone.
//   volunteers table      the SOURCE OF TRUTH the app reads and writes.
//
// Why not read the sheet directly on every request (which is what we did
// before): a published CSV is edge-cached by Google, so a fresh signup can take
// several minutes to appear — it looks exactly like a bug. The table is also
// the only place that can hold what the app learns later: notes, tags, history.
//
// Sync is deliberately additive. It never deletes: someone removed from the
// sheet keeps their registrations, and app-managed columns (`notes`) are never
// overwritten by a sync.

import { createClient } from "@supabase/supabase-js";
import { getVolunteers as readSheet } from "./sheets.js";
import { syncBeneficiaries } from "./beneficiaries.js";
import { markAttended, unmarkAttended } from "./loyalty.js";

let client = null;

export function volunteersConfigured() {
  return Boolean(
    process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY,
  );
}

function db() {
  if (!client) {
    client = createClient(
      process.env.SUPABASE_URL,
      process.env.SUPABASE_SERVICE_ROLE_KEY,
      { auth: { persistSession: false, autoRefreshToken: false } },
    );
  }
  return client;
}

function unwrap(result, what) {
  if (result.error) {
    throw new Error(`Supabase ${what} failed: ${result.error.message}`);
  }
  return result.data;
}

function toVolunteer(row) {
  return {
    id: row.id,
    name: row.name || "",
    phone: row.phone,
    type: row.type || "",
    pillars: row.pillars || [],
    roles: row.roles || [],
    // Which events this volunteer actually attended. Written by
    // markEventAttended below, alongside the points ledger; readiness reads
    // it to pick who to re-invite.
    eventsAttended: row.events_attended || [],
    // Volunteer Loyalty Programme (migration 007). Cached on the row, kept
    // in sync by lib/loyalty.js on every "mark attended".
    loyaltyPoints: row.loyalty_points || 0,
    loyaltyTier: row.loyalty_tier || "",
    notes: row.notes || "",
    // Standby who can be messaged by the backup-volunteer alert cron
    // (migration 010) — an organiser decision, never set by the sync loop.
    isBackup: row.is_backup || false,
  };
}

/**
 * Tag or untag a volunteer as a standby for backup-alerts — an organiser
 * decision made in the console, not something the sync loop ever touches.
 */
export async function setBackupFlag(phone, isBackup) {
  const result = await db()
    .from("volunteers")
    .update({
      is_backup: Boolean(isBackup),
      updated_at: new Date().toISOString(),
    })
    .eq("phone", phone)
    .select()
    .maybeSingle();
  const row = unwrap(result, "set backup flag");
  if (!row) throw new Error("No volunteer found for this phone number.");
  return toVolunteer(row);
}

/** Everyone in the table, newest signup last. */
export async function listVolunteers() {
  const rows = unwrap(
    await db()
      .from("volunteers")
      .select("*")
      .order("first_seen_at", { ascending: true }),
    "list volunteers",
  );
  return (rows || []).map(toVolunteer);
}

export async function countVolunteers() {
  const { count, error } = await db()
    .from("volunteers")
    .select("phone", { count: "exact", head: true });
  if (error)
    throw new Error(`Supabase count volunteers failed: ${error.message}`);
  return count || 0;
}

/**
 * Credit someone with having attended an event. THE single attendance writer.
 *
 * Two stores need to agree and previously didn't:
 *   - volunteers.events_attended  (migration 003) — what readiness reads for
 *     "re-invite past volunteers"
 *   - volunteer_event_log / beneficiary_activity_log (migration 007) — the
 *     points ledger and VIP threshold
 *
 * Writing only one leaves the other silently wrong: points with nobody to
 * re-invite, or a re-invite list worth no points. So this writes both, and
 * it is the only thing that does.
 *
 * Deliberately an organiser's decision on the roster after the event, not
 * something derived from a "yes" RSVP — an RSVP that turned into a no-show
 * shouldn't earn a credit or a VIP pass.
 *
 * Idempotent on both sides: the array skips a duplicate, and the log tables
 * have unique(person, event) with ignoreDuplicates.
 *
 * Works for beneficiaries too — they have no events_attended array, so only
 * the log side applies, which is what drives the VIP Pass.
 */
export async function markEventAttended(phone, eventName) {
  const existing = unwrap(
    await db()
      .from("volunteers")
      .select("events_attended")
      .eq("phone", phone)
      .maybeSingle(),
    "read volunteer for attendance",
  );

  let eventsAttended = existing?.events_attended || [];
  let alreadyMarked = false;

  if (existing) {
    if (eventsAttended.includes(eventName)) {
      alreadyMarked = true;
    } else {
      eventsAttended = [...eventsAttended, eventName];
      unwrap(
        await db()
          .from("volunteers")
          .update({
            events_attended: eventsAttended,
            updated_at: new Date().toISOString(),
          })
          .eq("phone", phone),
        "mark event attended",
      );
    }
  }

  // Points + VIP. Resolves volunteer OR beneficiary itself, and returns null
  // if the phone is in neither table.
  const loyalty = await markAttended({ campaign: eventName, phone });

  if (!existing && !loyalty) {
    throw new Error("No volunteer or beneficiary found for this phone number.");
  }

  return { alreadyMarked, eventsAttended, loyalty };
}

/**
 * Undo a credit given in error — the mirror of markEventAttended.
 *
 * Marking writes two stores, so unmarking has to clear both. Calling
 * unmarkAttended alone would drop the points but leave the event's name in
 * events_attended, which is what readiness reads for "re-invite past
 * volunteers" — the person would keep counting as a returning volunteer they
 * no longer are.
 *
 * Works for beneficiaries too: they have no array, so only the ledger applies,
 * which is what re-derives the VIP Pass.
 */
export async function unmarkEventAttended(phone, eventName) {
  const existing = unwrap(
    await db().from("volunteers").select("id,events_attended").eq("phone", phone).maybeSingle(),
    "read volunteer for unmark"
  );

  if (existing) {
    unwrap(
      await db()
        .from("volunteers")
        .update({
          events_attended: (existing.events_attended || []).filter(
            (n) => n !== eventName
          ),
          updated_at: new Date().toISOString(),
        })
        .eq("id", existing.id),
      "prune events_attended"
    );
  }

  const loyalty = await unmarkAttended({ campaign: eventName, phone });

  if (!existing && !loyalty) {
    throw new Error("No volunteer or beneficiary found for this phone number.");
  }
  return { loyalty };
}

/** Two profiles differ if anything the form controls has changed. */
function differs(existing, incoming) {
  const sameList = (a = [], b = []) =>
    a.length === b.length && a.every((v, i) => v === b[i]);
  return (
    (existing.name || "") !== (incoming.name || "") ||
    (existing.type || "") !== (incoming.type || "") ||
    !sameList(existing.pillars, incoming.pillars) ||
    !sameList(existing.roles, incoming.roles)
  );
}

/**
 * Pull the sheet across into the table.
 * Returns { added, updated, unchanged, total } so the UI can say what happened.
 */
/**
 * Which table does a sheet row belong in?
 *
 * A blank answer means the row predates the "I am a ..." question. Those default
 * to volunteer — the behaviour before beneficiaries existed — because silently
 * moving an existing person to another table would drop them out of the
 * recipient list they're already on.
 */
export function isBeneficiary(person) {
  return /^benef/i.test(String(person.type || "").trim());
}

export async function syncFromSheet() {
  // The volunteer form. Beneficiaries get their own form, and therefore their
  // own sheet — set BENEFICIARY_SHEET_CSV_URL when that exists.
  const fromSheet = await readSheet();
  const beneficiaryUrl = process.env.BENEFICIARY_SHEET_CSV_URL;
  const fromBeneficiarySheet = beneficiaryUrl
    ? await readSheet(beneficiaryUrl)
    : [];

  // A row with no usable phone can't be messaged or keyed on — skip it rather
  // than writing a row nothing can ever reach.
  const hasPhone = (p) => p.phone && p.phone.length >= 8;
  const usableAll = fromSheet.filter(hasPhone);

  const usable = usableAll.filter((v) => !isBeneficiary(v));
  // Anyone on the volunteer sheet who still answers "Beneficiary" is routed
  // across — a safety net for legacy rows, not the main path.
  const beneficiaries = [
    ...usableAll.filter(isBeneficiary),
    ...fromBeneficiarySheet.filter(hasPhone),
  ];

  const existing = unwrap(
    await db().from("volunteers").select("phone,name,type,pillars,roles"),
    "read volunteers for sync",
  );
  const byPhone = new Map((existing || []).map((r) => [r.phone, r]));

  const now = new Date().toISOString();
  const toInsert = [];
  const toUpdate = [];
  let unchanged = 0;

  for (const v of usable) {
    const incoming = {
      phone: v.phone,
      name: v.name || null,
      // Type comes from WHICH FORM they filled, not from a column: the
      // volunteer form has no "I am a" question any more, so a blank value
      // must not overwrite an existing "Volunteer" with null.
      type: v.type || "Volunteer",
      pillars: v.pillars || [],
      roles: v.roles || [],
    };
    const prior = byPhone.get(v.phone);

    if (!prior) {
      toInsert.push({ ...incoming, source: "form", updated_at: now });
    } else if (differs(prior, incoming)) {
      // `notes` and `events_attended` are absent here on purpose — they're
      // app-managed, and a sync must never clobber them.
      toUpdate.push({ ...incoming, updated_at: now });
    } else {
      unchanged += 1;
    }
  }

  if (toInsert.length > 0) {
    unwrap(await db().from("volunteers").insert(toInsert), "insert volunteers");
  }
  for (const row of toUpdate) {
    unwrap(
      await db().from("volunteers").update(row).eq("phone", row.phone),
      "update volunteer",
    );
  }

  const b = await syncBeneficiaries(beneficiaries);

  return {
    added: toInsert.length + b.added,
    updated: toUpdate.length + b.updated,
    unchanged: unchanged + b.unchanged,
    total: usableAll.length + fromBeneficiarySheet.length,
    skipped:
      fromSheet.length -
      usableAll.length +
      (fromBeneficiarySheet.length -
        fromBeneficiarySheet.filter(hasPhone).length),
    volunteers: usable.length,
    beneficiaries: beneficiaries.length,
  };
}
