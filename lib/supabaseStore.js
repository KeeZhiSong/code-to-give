...
/**
 * The event this person is most recently involved with, by their own reply.
 *
 * Someone who answered a poll yesterday and texts "what should I bring?" today
 * means THAT event — not whichever one happens to be soonest on the calendar.
 * Their own answer is the only per-person link we have once a message arrives
 * carrying no poll id, and it beats any global guess.
 *
 * Returns an event id, or null if they've never replied to anything.
 */
export async function lastEventIdForPhone(phone) {
