...
//
// The contract below says a SLOW call must cost nothing — but a try/catch only
// covers a call that fails, not one that never answers. Without this the
// fallback can hold the webhook open until the platform kills it, and GreenAPI
// reads that as a failed delivery: the vote is lost and the roster never
// updates. An inbound RSVP is worth more than an answered question, so the
// model gets a few seconds and then we move on without it.
const TIMEOUT_MS = Number(process.env.AI_FALLBACK_TIMEOUT_MS || 4000);
...
const SYSTEM_INSTRUCTION =
  ...
  "answer using ONLY the facts given below — or leave it empty if those " +
  "facts don't cover the question, per the response schema's field " +
  "description. Reply in whatever language the incoming message was " +
  "written in, not necessarily English. Respond with JSON matching the " +
  "given schema only.";
...
function eventFacts(event) {
  ...
  if (event.starts_at) lines.push(`Starts: ${event.starts_at}`);
  if (event.ends_at) lines.push(`Ends: ${event.ends_at}`);
 ...
 * of truth for any answer. Never throws AND never hangs — a slow or failing
 * model call must cost nothing more than falling back to "ignore quietly", the
 * behaviour before this existed. Returns null when unconfigured, empty, on
 * error, or when the model doesn't answer inside TIMEOUT_MS.
 */
...
Message: "${rawText}"`,
    ...
    const { intent, answer } = JSON.parse(res.text) || {};
    if (!["yes", "no", "question", "other"].includes(intent)) return null;
    return { intent, answer: typeof answer === "string" ? answer.trim() : "" };
  } catch {
    // Unconfigured, failed, or too slow — all three mean the same thing here:
    // ignore quietly, exactly as it did before this module existed.
    return null;
  } finally {
    clearTimeout(timer);
  }
