...
import { promoteNextWaitlisted } from "./waitlist.js";
...
async function isWaitlisted(campaign, phone) {
  try {
...
async function backfillIfSlotFreed(campaign, phone, log) {
  try {
    const event = await getEventByName(campaign);
    if (!event || event.capacity == null) return;

    const promoted = await promoteNextWaitlisted(event.id, event.name);
    if (promoted) {
      log(
        `⬆️  promoted ${promoted.name} (${promoted.phone}) off the waitlist for ${event.name}`,
      );
    }
  } catch (e) {
...
// Was this phone's last recorded answer for the campaign "yes"? Used to tell
// a genuine cancellation (frees a slot) from a first-time "no" (doesn't).
async function wasConfirmedYes(campaign, phone) {
  const rows = await getResponses(campaign);
  return rows.some((r) => r.phone === phone && r.answer === "yes");
}
...
export async function handleWebhook(body, { log = console.log } = {}) {
  ...
  switch (md.typeMessage) {
    case "pollUpdateMessage": {
      ...
      if (!choice) {
        ...
        // answer.
        const wasYes = await wasConfirmedYes(campaign, phone);
        await clearResponse({ phone, campaign });
        log(`↩️  ${name} retracted their vote`);
        if (wasYes) await backfillIfSlotFreed(campaign, phone, log);
  ...
  // overwrite so there's still a prior answer to compare against.
  const wasYes = answer === "no" && (await wasConfirmedYes(campaign, phone));
  ...
  log(
    `${answer === "yes" ? "✅" : "❌"} ${name} → ${answer.toUpperCase()}  ("${rawAnswer}")`,
  );

  if (wasYes) await backfillIfSlotFreed(campaign, phone, log);

  const waitlisted = answer === "yes" && (await isWaitlisted(campaign, phone));
  ...
  try {
    await sendText(
      ...
      waitlisted
        ...
        : await confirmationFor(campaign, answer),
    );
  } catch (e) {
