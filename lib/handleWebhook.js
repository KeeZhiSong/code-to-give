...
export async function handleWebhook(body, { log = console.log } = {}) {
  ...
  switch (md.typeMessage) {
    ...
    case "extendedTextMessage": {
      ...
      // A quoted reply carries the id of the message being replied to. If that
      // was one of our polls, this is an EXACT match — the volunteer literally
      // pointed at the event they mean. Free, and better than any inference.
      const quoted = md.extendedTextMessageData?.stanzaId;
      if (quoted) {
        const viaQuote = await eventForStanza(quoted);
        if (viaQuote) {
          campaign = viaQuote.name || campaign;
          eventId = viaQuote.id;
        }
      }
      break;
  ...
  if (!answer) {
    ...
    // Which event are they actually asking about? Best evidence first.
    //
    //   1. an exact id — they voted on, or replied quoting, a specific poll
    //   2. their OWN most recent RSVP — they answered it, so they mean it
    //   3. the next upcoming event — a guess, but a reasonable one
    //
    // Without 2, someone who said yes to Sunday's drive and then asked "what
    // should I bring?" would be answered about whatever event happened to be
    // soonest, which could be a different one entirely.
    let event = null;
    try {
      event = await eventFor(campaign, eventId);
      if (!event) {
        const theirs = await lastEventIdForPhone(phone);
        if (theirs) event = await getEvent(theirs);
      }
