 ...
 *
 * Built here, not by the model: the model is never trusted to state a phone
 * number, so a wrong one can't reach a volunteer. Falls back to
 * DEFAULT_CONTACT_PHONE when the event has no on-the-day contact set, same as
 * every other optional field in this file.
 */
export function formatNeedsHuman(event) {
  const contact = [event?.contact_name?.trim(), event?.contact_phone?.trim()]
    .filter(Boolean)
    .join(" · ");
  if (contact) {
    return `Good question — that's not something I can answer for you, but ${contact} can help. Feel free to reach out. 🙏`;
  }
  // With no contact configured anywhere, promise a follow-up rather than
  // printing a blank where a number should be.
  return DEFAULT_CONTACT_PHONE
    ? `Good question — that's not something I can answer for you. Please reach out to ${DEFAULT_CONTACT_PHONE} for help. 🙏`
    : "Good question — that's not something I can answer for you. One of our organisers will follow up with you shortly. 🙏";
}
