...
import { VIP_THRESHOLD_ATTENDANCES, tierFor, nextTierFor } from "./config.js";
...
async function markVolunteerAttended({ person, event, phone, points }) {
  ...
  unwrap(
    await db().from("volunteer_event_log").upsert(
      {
  ...
  // send succeeds.
  //
  // Dynamic import: poster.jsx needs JSX transpilation, which only Next's
  // toolchain provides. scripts/listen.js runs under plain `node` (no
  // transpiler) and pulls in this module transitively via handleWebhook.js —
  // a static top-level import of a .jsx file would crash it on startup even
  // though attendance marking (the only place this path fires) never runs
  // from the listener.
  let notified = false;
  try {
    const { renderVolunteerMilestoneImage } = await import("./poster.jsx");
...
async function markBeneficiaryAttended({ person, event, phone, points }) {
  unwrap(
    await db().from("beneficiary_activity_log").upsert(
      {
  ...
  if (justBecameVip) {
    try {
      const { renderVipPassImage } = await import("./poster.jsx");
