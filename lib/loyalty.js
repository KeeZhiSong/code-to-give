...
import { renderVipPassImage, renderVolunteerMilestoneImage } from "./poster.js";
import {
...
async function markVolunteerAttended({ person, event, phone, points }) {
  ...
  // send succeeds.
  //
  // Statically imported at the top now that poster.js is plain
  // React.createElement rather than JSX. The dynamic import that used to be
  // here existed only so scripts/listen.js — plain `node`, no JSX transform —
  // could pull this module in transitively without crashing on startup; a
  // module that needs no transpiling removes the reason for the workaround.
  let notified = false;
  try {
