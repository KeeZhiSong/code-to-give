...
/** Postgres/PostgREST saying a column in the payload doesn't exist. */
...
/**
 * Run a write, and if the database hasn't had migration 008 yet, retry without
 * the columns it adds.
 *
...
async function withoutMissingDetails(data, run) {
  ...
  const reduced = { ...data };
  for (const key of DETAIL_COLUMNS) delete reduced[key];
  return run(reduced);
}
