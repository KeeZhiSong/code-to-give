 ...
 * message — used for the VIP Pass / volunteer milestone poster (lib/poster.js).
 *
...
// Only ever pass the fields you mean to change — GreenAPI's setSettings
// replaces each field it's given, but every field you omit stays as-is, so
// there's no risk of blanking incomingWebhook/pollMessageWebhook by mistake
// when the only intent is toggling webhookUrl.
export async function setSettings(fields) {
  return callApi("setSettings", fields, "POST");
}
